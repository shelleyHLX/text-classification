...
from models.biblosa.nn_utils.general import flatten, reconstruct, exp_mask, add_reg_without_bias,\
    exp_mask_for_high_rank, mask_for_high_rank, add_var_reg
from models.biblosa.nn_utils.basic import selu
...
def _linear(xs,output_size,bias,bias_start=0., scope=None):
    with tf.variable_scope(scope or 'linear_layer'):
        ...
        W = tf.get_variable('W', shape=[input_size,output_size],dtype=tf.float64,
                            )
        if bias:
            bias = tf.get_variable('bias', shape=[output_size],dtype=tf.float64,
