...
def mask_for_high_rank(val, val_mask, name=None):
    ...
    return tf.multiply(val, tf.cast(val_mask, tf.float64), name=name or 'mask_for_high_rank')
...
def exp_mask_for_high_rank(val, val_mask, name=None):
    val_mask = tf.expand_dims(val_mask, -1)
    return tf.add(val, (1 - tf.cast(val_mask, tf.float64)) * VERY_NEGATIVE_NUMBER,
