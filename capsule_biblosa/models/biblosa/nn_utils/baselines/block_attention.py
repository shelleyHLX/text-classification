...
from models.biblosa.nn_utils.nn import bn_dense_layer, linear
...
def simple_block_attention(
    ...
    with tf.variable_scope(scope or 'block_simple'):
        ...
        with tf.variable_scope('split_seq'):
            block_num = tf.cast(tf.ceil(tf.divide(tf.cast(sl, tf.float64), tf.cast(block_len, tf.float64))), tf.int32)
            comp_len = block_num * block_len - sl

            rep_tensor_comp = tf.concat([rep_tensor, tf.zeros([bs, comp_len, ivec], tf.float64)], 1)
