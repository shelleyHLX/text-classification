...
import model_context_fusion
from models.biblosa.configs import cfg
# model_context_fusion import ModelContextFusion as Model
...
def train_epoch(data_path, sess, model, train_fetches,
    ...
    for batch in tqdm(range(n_tr_batches)):
        ...
        y_batch = to_categorical(y_batch, cfg.n_class)
        _batch_size = len(y_batch)
        feed_dict = {model.token_seq: X_batch, model.gold_label: y_batch, model.is_train: True}
        ...
        if 0 == (global_step + 1) % 500:
            ...
            _batch_size = len(y_batch)
            feed_dict = {model.token_seq: X_batch, model.gold_label: y_batch, model.is_train: False}
            _cost = sess.run(valid_fetches, feed_dict)
...
def main(_):
    ...
    with tf.Session(config=config) as sess:
        model = model_context_fusion.ModelContextFusion(W_embedding, 'biblosa')
        with tf.variable_scope('training_ops') as vs:
            learning_rate = tf.train.exponential_decay(FLAGS.lr, model.global_step,
                                                       FLAGS.decay_step, FLAGS.decay_rate, staircase=True)
        ...
        for epoch in range(FLAGS.max_max_epoch):
            ...
            train_fetches = [model.loss, train_op,]  # update_op]
