...
class BidirectionalLanguageModelGraph(object):
    ...
    def _build_word_embeddings(self):
        ...
        # the word embeddings
        with tf.device("/cpu:0"):  # , tf.variable_scope('abcd', reuse=tf.AUTO_REUSE):
            embed_path = './bilm_trained/glove_200.txt'
            import gensim
            ...
            embedding_weights = tf.get_variable("embedding", [self._n_tokens_vocab, projection_dim],
                dtype=DTYPE,  initializer=tf.constant_initializer(tmp_embed),trainable=True,)
            self.embedding = tf.nn.embedding_lookup(embedding_weights, self.ids_placeholder)
    ...
    def _build_lstms(self):
        ...
        for direction in ['forward', 'backward']:
            ...
            for i in range(n_lstm_layers):
                ...
                init_states = [tf.Variable(tf.zeros([self._max_batch_size, dim]), trainable=False)
                ...
                with tf.variable_scope(variable_scope_name):
                    layer_output, final_state = tf.nn.dynamic_rnn(lstm_cell, layer_input, sequence_length=sequence_lengths,
                ...
                if direction == 'forward':
                    ...
                    self.lstm_outputs[direction].append(tf.reverse_sequence(
                        layer_output, sequence_lengths, seq_axis=1, batch_axis=0))

                with tf.control_dependencies([layer_output]):
                    # update the initial states
                    for i in range(2):
                        new_state = tf.concat([final_state[i][:batch_size, :], init_states[i][batch_size:, :]], axis=0)
    ...
    with tf.Session(config=config) as sess:
        ...
        with open(dataset_file, 'r') as fin, h5py.File(outfile, 'w') as fout:
            for line in fin:
                ...
                ds = fout.create_dataset('{}'.format(sentence_id), embeddings.shape[1:], dtype='float32',
                    data=embeddings[0, :, :, :])
