...
flags.DEFINE_integer('valid_step', 500, 'valid_step, default: 2500')
...
def valid_epoch(data_path, sess, model, bilm, elmo_context_input, context_token_ids, sess_bilm, ):
    ...
    for i in range(n_va_batches):
        ...
        elmo_context_vector = get_elmo(X_batch, bilm, elmo_context_input, context_token_ids, sess_bilm)
        marked_labels_list.extend(y_batch)
        y_batch = to_categorical(y_batch, settings.n_class)
        _batch_size = len(y_batch)
        fetches = [model.loss, model.y_pred]
        feed_dict = {model.X_inputs: elmo_context_vector,
...
def get_elmo(X_batch, bilm, elmo_context_input, context_token_ids, sess):
...
def train_epoch(data_path, sess, model, bilm, elmo_context_input, context_token_ids, sess_bilm,
                train_fetches, valid_fetches, train_writer, test_writer, logger):
    global last_score12
    global lr
    time0 = time.time()
    ...
    for batch in tqdm(range(n_tr_batches)):
        ...
        if 0 == (global_step + 1) % FLAGS.valid_step:
            f1_micro, f1_macro, score12 = valid_epoch(data_valid_path, sess, model,
                                                      bilm, elmo_context_input, context_token_ids, sess_bilm, )
        ...
        elmo_context_vector = get_elmo(X_batch, bilm, elmo_context_input, context_token_ids, sess_bilm)
        # print('elmo', elmo_context_vector.shape)
        # exit(0)
        y_batch = to_categorical(y_batch, settings.n_class)
        _batch_size = len(y_batch)
        feed_dict = {model.X_inputs: elmo_context_vector,
...
def main(_):
    ...
    with tf.Session(config=config,) as sess:
        ...
        for epoch in range(FLAGS.max_max_epoch):
            ...
            train_epoch(data_train_path, sess, model, bilm, elmo_context_input, context_token_ids, sess_bilm,
                        train_fetches, valid_fetches, train_writer, test_writer, logger)
        # 最后再做一次验证
        f1_micro, f1_macro, score12 = valid_epoch(data_valid_path, sess, model,
                                                  bilm, elmo_context_input, context_token_ids, sess_bilm, )
