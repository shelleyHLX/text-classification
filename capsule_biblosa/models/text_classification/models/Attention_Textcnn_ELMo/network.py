...
class Settings(object):
    def __init__(self):
        self.model_name = 'Attention_TextCNN_ELMo'
        self.fact_len = 198
...
class Atten_TextCNN(object):
    def __init__(self, W_embedding, settings):
        ...
        with tf.name_scope('Inputs'), tf.device('/cpu:0'):
            self._X_inputs = tf.placeholder(tf.float32, [None, self.fact_len, 400], name='X_input')
        ...
        with tf.variable_scope('fc-bn-layer'):
            W_fc = self.weight_variable([w, w], name='Weight_fc')
            tf.summary.histogram('W_fc', W_fc)
            h_fc = tf.matmul(output, W_fc, name='h_fc')
            beta_fc = tf.Variable(tf.constant(0.1, tf.float32, shape=[w], name="beta_fc"))
        ...
        with tf.variable_scope('out_layer'):
            W_out = self.weight_variable([w, self.n_class], name='Weight_out')
            tf.summary.histogram('Weight_out', W_out)
            b_out = self.bias_variable([self.n_class], name='bias_out')
            tf.summary.histogram('bias_out', b_out)
            self.logits = tf.nn.xw_plus_b(self.fc_bn_relu, W_out, b_out, name='logits')  # 每个类别的分数 scores
            self._y_pred = tf.sigmoid(self.logits, name='y_pred')
    ...
    def _inference(self, X_inputs):
        ...
        out = self.task_specific_attention(X_inputs, self.embedding_size)
        print('out', out)
