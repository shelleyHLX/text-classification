...
from models.capsule.utils_cap import _conv2d_wrapper, _get_weights_wrapper
...
def capsule_conv_layer(poses_init, activations_init, shape, strides, iterations, name):
    with tf.variable_scope(name):               # shape=[3, 1, 16, 16]
        ...
        print('poses ', poses.get_shape().as_list(), type(poses))
        print('poses ', activations.get_shape().as_list(), type(activations))
