...
n_tr_batches = len(tr_batches) - 1
n_va_batches = len(va_batches) - 1


def get_batch(data_path, batch_id):
    new_batch = np.load(data_path + str(batch_id) + '.npz')
    X_batch = new_batch['X']
    y_batch = new_batch['acc']
...
def main(_):
    ...
    with tf.Session(config=config) as sess:
        model = network2.capusle(W_embedding, settings)
        ...
        if os.path.exists(ckpt_path + "checkpoint"):
            ...
            cost, f1_micro, f1_macro, score12 = valid_epoch(data_valid_path, sess, model)
            print('cost=%g, f1_micro=%g, f1_macro=%g, score12=%g' % (cost, f1_micro, f1_macro, score12))
