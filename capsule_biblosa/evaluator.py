...
def to_categorical_single_class(cl, num_class):
...
def cail_evaluator(predict_labels_list, marked_labels_list):
    ...
    for i in range(samples):  # number of samples
        predict_category = [1 if i > 0.5 else 0 for i in predict_labels_list[i]]
    ...
    for i in range(samples):
        marked_category = to_categorical_single_class(marked_labels_list[i], num_class)
