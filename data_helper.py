...
def to_categorical(labels, num_class):
    n_sample = len(labels)
    y = np.zeros(shape=(n_sample, num_class))

    for i in range(n_sample):
        topic_index = labels[i]
        y[i, topic_index] = 1
    return y

def to_categorical_death(labels, num_class):
    n_samples = len(labels)
    y = np.zeros(shape=(n_samples, num_class))

    for i in range(n_samples):
        topic_index = int(labels[i])
        y[i, topic_index] = 1
    return y
