...
data_train_path = '../../data/capsule_wv256_bs128/train/'
data_valid_path = '../../data/capsule_wv256_bs128/valid/'
...
def get_batch(data_path, batch_id):
    ...
    X_batch = new_batch['X']
    acc = new_batch['acc']
    sent_len = new_batch['sent_len']
    length = new_batch['length']
    # death = new_batch['death']
    # imp = new_batch['imp']
    # lif = new_batch['lif']
    return [X_batch, acc, sent_len, length]  # law, death, imp, lif]
...
def valid_epoch(data_path, sess, model):
    ...
    for i in range(n_va_batches):
        [X_batch, y_batch, sent_len, length] = get_batch(data_path, i)
        marked_labels_list.extend(y_batch)
        y_batch = to_categorical(y_batch)
        _batch_size = len(y_batch)
        fetches = [model.loss, model.y_pred]
        feed_dict = {model.X_inputs: X_batch, model.wNum: sent_len, model.sNum: length,
                     model.y_inputs: y_batch, model.batch_size: _batch_size,
                     model.tst: True, model.is_train: False}
...
def train_epoch(data_path, sess, model, train_fetches,
    ...
    for batch in tqdm(range(n_tr_batches)):
        ...
        [X_batch, y_batch, sent_len, length] = get_batch(data_path, batch_id)
        y_batch = to_categorical(y_batch)
        _batch_size = len(y_batch)
        feed_dict = {model.X_inputs: X_batch, model.wNum: sent_len, model.sNum: length,
                     model.y_inputs: y_batch, model.batch_size: _batch_size,
                     model.tst: False, model.is_train: True}
        ...
        if 0 == (global_step + 1) % 500:
            ...
            [X_batch, y_batch, sent_len, length] = get_batch(data_valid_path, batch_id)
            y_batch = to_categorical(y_batch)
            _batch_size = len(y_batch)
            feed_dict = {model.X_inputs: X_batch, model.wNum: sent_len, model.sNum: length,
                         model.y_inputs: y_batch, model.batch_size: _batch_size,
