...
embedding_path2 = '../../data/glove_sameid_100.npy'
...
def valid_epoch(data_path, sess, model):
    ...
    for i in range(n_va_batches):
        ...
        feed_dict = {model.X_inputs: X_batch,
                     ...
                     model.tst: True, model.keep_prob: 1.0, model.is_train: False}
...
def train_epoch(data_path, sess, model, train_fetches,
    ...
    for batch in tqdm(range(n_tr_batches)):
        ...
        feed_dict = {model.X_inputs: X_batch,
                     model.y_inputs: y_batch, model.batch_size: _batch_size,
                     model.tst: False, model.keep_prob: FLAGS.keep_prob, model.is_train: True}
...
def main(_):
    ...
    W_embedding2 = np.load(embedding_path2)
    ...
    with tf.Session(config=config) as sess:
        model = network.Atten_TextCNN(W_embedding, W_embedding2, settings)
