...
class Atten_TextCNN(object):
    ...
    def conv_layers_leNet5(self, input_x, name_scope, reuse_flag=False):
        ...
        for i, filter_size in enumerate(self.filter_sizes):
            with tf.variable_scope(str(name_scope) + "convolution-pooling-%s" % filter_size, reuse=reuse_flag):
                #1) CNN->BN->relu
                filter = tf.get_variable("filter-%s" % filter_size, [filter_size, self.embedding_size, 1, self.n_filter],
                ...
                cnn=tf.contrib.layers.batch_norm(conv,is_training=self.is_train,scope='cnn1')
                print("conv1:", conv)
                ...
                h=tf.reshape(h,[-1,self.fact_len-filter_size+1,self.n_filter,1]) #shape:[batch_size,sequence_length-filter_size+1,num_filters,1]
                #Layer2:CONV-RELU
                ...
                #3. Max-pooling
                pooling = tf.nn.max_pool(h, ksize=[1, (self.fact_len - filter_size*2+2), 1, 1],strides=[1, 1, 1, 1],
    ...
    def _inference(self):
        input = tf.nn.embedding_lookup(self.embedding, self.X_inputs) #[batch_size,total_sequence_length,embed_size]
        # input2 = tf.nn.embedding_lookup(self.embedding2, self.X_inputs)
        inputs = self.conv_layers_leNet5(input, self.fact_len)
        # exit(0)
        return inputs
