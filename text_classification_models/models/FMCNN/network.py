...
class Atten_TextCNN(object):
    ...
    def TextCNN(self, X_inputs, n_step):
        ...
        for i, filter_size in enumerate(self.filter_sizes):
            with tf.variable_scope("conv-maxpool-%s" % filter_size):
                ...
                cons.append(tf.squeeze(h, axis=2))
        ...
        for i, filter_size in enumerate(self.filter_sizes):
            with tf.variable_scope("conv-maxpool-2-%s" % filter_size):
                ...
                bs, n_step, num_filt, _ = inputs.get_shape().as_list()
                print('bs, n_step, num_filt ', bs, n_step, num_filt)
                filter_shape = [filter_size, self.embedding_size, 1, self.n_filter]
                W_filter = self.weight_variable(shape=filter_shape, name='W_filter')
                beta = self.bias_variable(shape=[self.n_filter], name='beta_filter')
                tf.summary.histogram('beta', beta)
        ...
        h_pool_flat2 = tf.reshape(h_pool2, [-1, self.n_filter_total])
        output = [h_pool_flat, h_pool_flat2]
    ...
    def _inference(self, X_inputs):
        ...
        inputs = self.TextCNN(inputs, self.fact_len)
        return inputs
