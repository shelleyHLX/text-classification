...
class Atten_TextCNN(object):
    ...
    def conv3_highway_conv2d_layer(self, x, k, carry_bias=-1.0):
        W = tf.get_variable("W_conv%s" % k, [self.kernel_size, 1, self.num_filters, self.num_filters],
                            initializer=self.initializer, dtype=tf.float32)
        b = tf.Variable(tf.constant(0.1, shape=self.num_filters), name='bias')
        W_T = tf.get_variable("W_conv%s" % k, [self.kernel_size, 1, self.num_filters, self.num_filters],
                              initializer=self.initializer, dtype=tf.float32)
        b_T = tf.Variable(tf.constant(0.1, shape=self.num_filters), name='bias')
        H = tf.nn.relu(tf.nn.conv2d(x, W, strides=[1, 1, 1, 1], padding='SAME') + b, name='activation')
        T = tf.sigmoid(tf.nn.conv2d(x, W_T, strides=[1, 1, 1, 1], padding='SAME') + b_T, name='transform_gate')
    ...
    def inference(self):
        ...
        conv3 = self.conv3_highway_conv2d_layer(1, conv3_pre_activation)  # [batch,seq-3+1,1,250]
        # batch norm
        conv3 = tf.layers.batch_normalization(conv3)
