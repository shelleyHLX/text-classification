...
from models.Attention_Textcnn_loss.network import Atten_TextCNN, Settings
...
def valid_epoch(data_path, sess, model):
    ...
    for i in range(n_va_batches):
        ...
        fetches = [model.loss, model.labels]
...
def train_epoch(data_path, sess, model, train_fetches,
    ...
    for batch in tqdm(range(n_tr_batches)):
        ...
        if 0 == (global_step + 1) % FLAGS.valid_step:
            cost, f1_micro, f1_macro, score12 = valid_epoch(data_valid_path, sess, model)
            print('Global_step=%d: cost=%g, f1_micro=%g, f1_macro=%g, score12=%g, time=%g s' % (
                global_step, cost, f1_micro, f1_macro, score12, time.time() - time0))
            logger.info('END:Global_step={}: cost={}, f1_micro={}, f1_macro={}, score12={}'.
                        format(sess.run(model.global_step), cost, f1_micro, f1_macro, score12))
            time0 = time.time()
...
def main(_):
    ...
    with tf.Session(config=config) as sess:
        model = Atten_TextCNN(W_embedding, settings)
        ...
        if os.path.exists(ckpt_path + "checkpoint"):
            ...
            cost, f1_micro, f1_macro, score12 = valid_epoch(data_valid_path, sess, model)
            print('cost=%gf1_micro=%g, f1_macro=%g, score12=%g' % (cost, f1_micro, f1_macro, score12))
        ...
        # 最后再做一次验证
        cost, f1_micro, f1_macro, score12 = valid_epoch(data_valid_path, sess, model)
        print('END:Global_step=%d: cost=%g, f1_micro=%g, f1_macro=%g, score12=%g' % (
            sess.run(model.global_step), cost, f1_micro, f1_macro, score12))
