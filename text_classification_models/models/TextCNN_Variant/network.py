...
class Settings(object):
    def __init__(self):
        self.model_name = 'TextCNN_variant'
        self.fact_len = 200
...
class Atten_TextCNN(object):
    ...
    def conv_layers_return_2d_great(self, input_x, name_scope, reuse_flag=False):#great 81.3
        ...
        for i, filter_size in enumerate(self.filter_sizes):
            with tf.variable_scope(str(name_scope) + "convolution-pooling-%s" % filter_size, reuse=reuse_flag):
                ...
                bs, seq, _, num = h.get_shape().as_list()

                #2) CNN->BN->relu
                h=tf.reshape(h,[-1,seq,self.n_filter,1]) #shape:[batch_size,sequence_length-filter_size+1,num_filters,1]
                #Layer2:CONV-RELU
                ...
                bs, a, b, c = h.get_shape().as_list()
                #3. Max-pooling
                pooling_max = tf.nn.max_pool(h, ksize=[1, (self.fact_len - filter_size*2+2), 1, 1],strides=[1, 1, 1, 1],
    ...
    def _inference(self, X_inputs):
        ...
        textcnn_out = self.conv_layers_return_2d_great(inputs, self.fact_len)
        # exit(0)
        return textcnn_out
