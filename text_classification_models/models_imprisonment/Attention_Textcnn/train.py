...
data_train_path = '../../data/s200/train_all/'
...
label_impri_dict = {0:-2, 1:-1, 2:0, 3:1, 4:2, 5:4,
                    6:7, 7:12, 8:18, 9:26, 10:39,
                    11:63, 12:98, 13:143, 14:185}

"""
-2: 0, -1: 1, 0: 2, 1: 3, 2-3: 4, 
4-5: 5, 6-9: 6, 10-14: 7, 15-23: 8, 24-35: 9,
36-53: 10, 54-79: 11, 80-120: 12, 121-179: 13, 
180-300: 14
"""
...
def train_epoch(data_path, sess, model, train_fetches,
    ...
    for batch in tqdm(range(n_tr_batches)):
        ...
        if 0 == (global_step + 1) % FLAGS.valid_step:
            cost, score = valid_epoch(data_valid_path, sess, model)
            logger.info('END:Global_step={}: cost={},score={}'.
                        format(sess.run(model.global_step), cost,score))
            time0 = time.time()
            if score > last_score12:
                last_score12 = score
...
def main(_):
    ...
    with tf.Session(config=config) as sess:
        ...
        if os.path.exists(ckpt_path + "checkpoint"):
            ...
            cost, score = valid_epoch(data_valid_path, sess, model)
            print('cost=%g, score=%g' % (cost, score))
